...
public final class NetworkUtils {
    ...
    /*
     * WeatherCast was originally built to use OpenWeatherMap's API. However, we wanted to provide
...
private static final String api="1c85453bde692f2cf9558da3d9ee631b";
    ...
    /* The APPID paramter allows us to designate the API KEY */
  private static   final String APPID_PARAM = "appid";
     ...
     * 2) Later in WeatherCast, you are going to add an alternate method of allowing the user
    ...
    public static URL getUrl(Context context) {
        if (WeatherCastPreferences.isLocationLatLonAvailable(context)) {
            double[] preferredCoordinates = WeatherCastPreferences.getLocationCoordinates(context);
            double latitude = preferredCoordinates[0];
            double longitude = preferredCoordinates[1];
            return buildUrlWithLatitudeLongitude(latitude, longitude);
        } else {
            String locationQuery = WeatherCastPreferences.getPreferredWeatherLocation(context);
    ...
    public static String getResponseFromHttpUrl(URL url) throws IOException {
        ...
        try {
            ...
            scanner.close();
            Log.v("test3",url.toString());
            Log.v("test3",response);
            return response;
